...
def main(args):
    ...
    n = int(args.val_ratio * len(input_ids))
    train_input_ids = input_ids[:n]
    val_input_ids = input_ids[n:]
    val_batch_size = args.batch_size
    val_block_size = args.block_size
    if len(val_input_ids) // args.block_size == 0:
        val_batch_size = 1
        val_block_size = len(val_input_ids) - 1
    # Create the dataset and dataloader
    train_dataset = CausalLMDataset(train_input_ids, args.block_size)
    val_dataset = CausalLMDataset(val_input_ids, val_block_size)
