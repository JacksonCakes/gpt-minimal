...
class SelfAttention(nn.Module):
    ...
    def forward(self,x):
        ...
        attn = q @ k.transpose(-2,-1) # (bsz,seq_len,head_size) @ (bsz,head_size,seq_len) -> (bsz,seq_len,seq_len)
        attn = attn * 1.0/math.sqrt(k.size(-1)) # scale by 1/sqrt(d)
        # perform causal masking to prevent model "see" future tokens
        masked_attn = attn.masked_fill(self.trill[:seq_len,:seq_len]==0,float('-inf')) # (bsz,seq_len,seq_len)
        # compute softmax
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self,num_embeddings,num_heads,head_size,block_size):
        ...
        self.m_heads = nn.ModuleList(
            SelfAttention(num_embeddings,head_size,block_size) for n in range(num_heads)
        )
...
class TransformerBlock(nn.Module):
    def __init__(self,num_embeddings,num_heads,head_size,block_size,hs_scale_factor):
        super().__init__()
...
class GPT(nn.Module):
    ...
    hs_scale_factor
        ...
        self.block = nn.Sequential(
            *[TransformerBlock(num_embeddings,num_heads,head_size,block_size,hs_scale_factor) for n in range(num_layer)]
        )
