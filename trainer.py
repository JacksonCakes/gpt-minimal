...
class Trainer:
    def __init__(self, device, model, optimizer, scheduler=None):
        self.device = device
    ...
    def train(
        self, train_loader, val_loader, tokenizer, vocab, max_steps, epoch, eval_iters
    ):
        ...
        for e in tqdm(range(epoch)):
            for inputs, labels in train_loader:
                ...
                if cur_steps % eval_iters == 0:
                    ...
                    print(
                        f"step {cur_steps}: train loss {train_loss:.4f}, val loss {val_loss:.4f}"
                    )
                    print(
                        self.model.generate(
                            prompt="At the core of LLMs",
                            tokenizer=tokenizer,
                            vocab=vocab,
                            max_new_tokens=100,
                            device=self.device,
